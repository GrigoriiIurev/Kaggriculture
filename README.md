...
in Google Colab and choose **Runtime -> Run all**. The notebook requests Google
Drive access before training, downloads the public
[Boatlee V16-RC5 agent](https://www.kaggle.com/code/boatlee/v16-rc5-high-score-8c-4s-premium-market-lead),
...
The production agent is `main.py`. Source code lives under
`src/kaggriculture/`, replay data under `data/`, and generated models and
reports under `artifacts/`. Behavior-cloning prototypes are kept in
`experiments/` and cannot replace the production agent without winning a
full-game validation tournament.

See [docs/WORKFLOW.md](docs/WORKFLOW.md) for the four daily commands: download
replays, build datasets, train, evaluate, and package a submission.
